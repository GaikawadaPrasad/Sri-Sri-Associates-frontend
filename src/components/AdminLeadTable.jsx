...
import { User, IndianRupee, ChevronDown, ArrowUpRight } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import toast from 'react-hot-toast';

const AdminLeadTable = ({ leads, onUpdate }) => {
  const navigate = useNavigate();
  const statusOptions = ['NEW LEAD', 'Approved', 'Disbursed', 'Rejected'];

  const handleStatusChange = async (e, leadId, newStatus) => {
    e.stopPropagation(); // Prevent navigating to details page when changing status
    try {
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-gray-50/50">
            <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Customer & Agent</th>
            <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Loan Information</th>
            ...
            >
              <td className="px-8 py-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-black text-slate-800 uppercase text-sm">{lead.customerName}</p>
              ...
              <td className="px-8 py-6">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-gray-400 uppercase mt-0.5">{lead.loanType}</p>
                </div>
              </td>
              ...
              <td className="px-8 py-6 text-right">
                <div className="flex items-center justify-end gap-3">
                  <div className="relative inline-block" onClick={(e) => e.stopPropagation()}>
                    <select 
                      value={lead.status}
                      onChange={(e) => handleStatusChange(e, lead._id, e.target.value)}
      ...
      {leads.length === 0 && (
        <div className="p-20 text-center">
          <p className="text-xs font-black text-gray-400 uppercase tracking-widest">No matching leads found</p>
        </div>
      )}
    </div>
  );
