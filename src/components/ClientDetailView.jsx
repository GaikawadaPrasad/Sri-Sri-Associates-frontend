...
import { 
  ...
  Target, Clock, Calendar, User, Mail 
} from "lucide-react";
...
const ClientDetailView = ({ client, onBack }) => {
  ...
  if (loading) return <div className="p-20 text-center font-black animate-pulse">LOADING AGENT PROFILE...</div>;

  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
      {/* Header */}
      <div className="flex items-center gap-4 mb-8">
        <button onClick={onBack} className="p-3 bg-white rounded-2xl shadow-sm hover:bg-slate-900 hover:text-white transition-all">
          <ArrowLeft size={20} />
        </button>
        <div>
          <h2 className="text-2xl font-black uppercase text-slate-900">{client.name}</h2>
          <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Agent ID: {client._id.slice(-6)}</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm">
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <h3 className="font-black text-slate-900 uppercase">{client.name}</h3>
              <p className="text-xs text-gray-400 font-bold">{client.email}</p>
            </div>
            
            <div className="space-y-3">
              <StatRow label="Total Leads" value={stats.total} icon={<FileText size={14}/>} color="blue" />
              <StatRow label="Disbursed" value={stats.disbursed} icon={<CheckCircle size={14}/>} color="green" />
              <StatRow label="Rejected" value={stats.rejected} icon={<XCircle size={14}/>} color="red" />
              <StatRow label="Pending" value={stats.pending} icon={<Clock size={14}/>} color="yellow" />
            </div>
          </div>
          ...
          <div className="bg-[#0A1D37] p-8 rounded-[40px] text-white shadow-xl">
             <div className="flex items-center gap-2 mb-6">
                ...
                <h4 className="text-xs font-black uppercase tracking-widest">Monthly Target</h4>
             </div>
             {target ? (
               <>
                 <div className="text-3xl font-black mb-2">₹{target.completedAmount?.toLocaleString()}</div>
                 <p className="text-[10px] font-bold text-gray-400 uppercase mb-4">Goal: ₹{target.targetAmount?.toLocaleString()}</p>
                 <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-blue-500" 
                      style={{ width: `${(target.completedAmount / target.targetAmount) * 100}%` }}
                    />
                 </div>
               </>
             ) : <p className="text-xs text-gray-500 italic">No target set for this month</p>}
          </div>
        </div>
            ...
            <div className="overflow-y-auto max-h-[600px]">
              <table className="w-full text-left">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>
                    <th className="p-4 text-[9px] font-black uppercase text-gray-400 pl-8">Date</th>
                    <th className="p-4 text-[9px] font-black uppercase text-gray-400">In</th>
                    <th className="p-4 text-[9px] font-black uppercase text-gray-400">Out</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {attendance.map((log) => (
                    <tr key={log._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4">
                        <span className={`text-[8px] font-black uppercase px-2 py-1 rounded-md ${log.status === 'IN' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'}`}>
                          {log.status}
                        </span>
                      </td>
                    </tr>
              ...
              {attendance.length === 0 && <div className="p-20 text-center text-gray-400 text-xs font-bold uppercase">No records found</div>}
            </div>
          </div>
        </div>
      </div>
