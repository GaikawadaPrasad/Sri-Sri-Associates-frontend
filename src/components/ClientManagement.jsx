    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredClients.map((client) => (
          ...
          >
            <div
              className={`absolute top-0 right-0 w-2 h-full ${client.isActive ? "bg-green-500" : "bg-red-500"}`}
            ></div>
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button
                onClick={() => {
                  toggleStatus(client._id, client.isActive);
                  toast.info(
                    `Agent ${client.isActive ? "deactivated" : "activated"}`,
                  );
                }}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-8 rounded-2xl w-full max-w-md">
            ...
            <p className="text-sm text-gray-600 mb-6">
              Setting target for{" "}
              <span className="font-bold">{selectedClient.name}</span> for the
              current month.
            </p>
